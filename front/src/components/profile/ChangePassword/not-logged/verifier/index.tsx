...
export function CodeVerifierPassword() {
  ...
  async function verifierHandle({ code, newpassword }: InsertCodeType) {
    ...
    try {
      ...
      const {response} = result.data as ChangePasswordResponseType;

      if (response) {
        setInsertMessage(<CustomAlert title="Sucesso!" msg="Senha alterada com sucesso" type={CustomAlertType.SUCESS} />);
        setTimeout(() => location.reload(), 1500)
      } else {
        const message = <CustomAlert type={CustomAlertType.ERROR} title="Erro ao alterar a senha:" msg={response || "ERRO DESCONHECIDO!"} />;
